...
def process_llm_response_old(user_input, show_references):
...
def process_llm_response(user_input, show_references, conversation_history):
    if show_references:
        llm_response = tools.answer_query(user_input, conversation_history)
        formatted_answer = tools.format_answer(llm_response)
        return formatted_answer
    else:
        llm_response = tools.answer_query_old(user_input, conversation_history)
        return llm_response
...
def chatbot_page():
    ...
    if user_input:
        ...
        assistant_response = process_llm_response(
            user_input, 
            st.session_state.get("show_references", True),
            st.session_state.messages
        )
...
def products_page():
    ...
    """, unsafe_allow_html=True)
        ...
        product_options = ["Selecciona un producto", "AI Chatbot", "Video extract", "Document extract"]
...
def display_product(product):
    ...
    if product == "AI Chatbot":
        chatbot_page()
    elif product == "Document extract":
        invoice_extraction_page()
    elif product == "Video extract":
        video_extraction_page()
...
def invoice_extraction_page():
    ...
    if uploaded_image is not None:
        ...
        # Mostrar miniatura de la imagen con un tamaño pequeño
        st.image(image_bytes, caption="Imagen cargada (clic para ampliar)", width=200)
        
        # Control interactivo para mostrar/ocultar imagen completa
        if st.checkbox("Ver imagen en tamaño completo"):
            st.image(image_bytes, caption="Imagen completa", use_column_width=True)
...
def video_extraction_page():
    ...
    # Inicializar la variable de sesión para guardar la transcripción
    ...
    if uploaded_video is not None:
        ...
        # Si aún no se ha transcrito el audio, proceder con la transcripción
        if st.session_state.transcribed_text is None:
            ...
            # Procesar el archivo de audio con AWS Transcribe
            with st.spinner("Transcribiendo el audio..."):
                st.session_state.transcribed_text = tools.transcribe_audio(audio_s3_uri)
        ...
        if st.session_state.transcribed_text:
            ...
            # Extraer palabras clave con Claude
            if st.button("Extraer palabras clave"):
                keywords = tools.extract_keywords_with_claude(st.session_state.transcribed_text)
                st.write("Palabras clave:")
                st.write(keywords)

            # Generar resumen con Claude
            if st.button("Generar resumen"):
                summary = tools.summarize_text_with_claude(st.session_state.transcribed_text)
                st.write("Resumen del texto:")
                st.write(summary)
